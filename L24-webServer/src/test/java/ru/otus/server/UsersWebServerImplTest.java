...
class UsersWebServerImplTest {
    ...
    private static final String API_ADMIN_URL = "client";
    ...
    static void setUp() throws Exception {
        ...
        AdminAuthService userAuthService = mock(AdminAuthService.class);

        given(userAuthService.authenticate(DEFAULT_ADMIN_LOGIN, DEFAULT_ADMIN_PASSWORD)).willReturn(true);
        given(userAuthService.authenticate(INCORRECT_USER_LOGIN, DEFAULT_ADMIN_PASSWORD)).willReturn(false);
        DBServiceClient dbServiceClient = mock(DBServiceClient.class);
        webServer = new UsersWebServerWithFilterBasedSecurity(WEB_SERVER_PORT, dbServiceClient, templateProcessor, userAuthService);
        webServer.start();
    }
    ...
    void shouldReturnForbiddenStatusForUserRequestWhenUnauthorized() throws Exception {
        HttpRequest request = HttpRequest.newBuilder().GET()
                .uri(URI.create(buildUrl(WEB_SERVER_URL, API_ADMIN_URL)))
                .build();
    ...
    void shouldNotReturnJSessionIdWhenLoggingInWithIncorrectData() throws Exception {
        HttpCookie jSessionIdCookie = login(buildUrl(WEB_SERVER_URL, LOGIN_URL), INCORRECT_USER_LOGIN, DEFAULT_ADMIN_PASSWORD);
